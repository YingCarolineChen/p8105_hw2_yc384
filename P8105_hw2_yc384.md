...
From the above steps, we can see that the values for variable
childs\_first\_name are not consistant. Some names were capitalized only
with the first letter. Some names are all entered as upper case. In
order to be consistent, we need to change the names to upper case.
...
``` r
baby_names %>% 
  group_by(birth_year, gender, ethnicity) %>% 
  summarize(
    n_obs = n(),
...
```
    ...
    ## # A tibble: 48 x 5
    ## # Groups:   birth_year, gender [12]
    ##   birth_year gender ethnicity                  n_obs duplicate_rows
    ##        <dbl> <chr>  <chr>                      <int>          <int>
    ## 1       2011 FEMALE ASIAN AND PACIFIC ISLANDER   310            156
    ## 2       2011 FEMALE BLACK NON HISPANIC           365            184
    ## 3       2011 FEMALE HISPANIC                     645            324
    ## # … with 45 more rows

``` r
baby_names_tidy = baby_names
distinct(baby_names_tidy)
```
...
#### From the about results, we can see that there are some duplicate rows. The following code chuck will create another dataset called baby\_names\_tidy to have those duplicate rows removed. The total number of rows dropped from 19418 - 19418 to 19418 - 12181.
